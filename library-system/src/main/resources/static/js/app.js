        ...
        <div class="card" style="margin-bottom: 12px; padding: 16px; border: 1px solid var(--border-color);">
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
                ...
                <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    ${!isRet ? `
                        <button class="btn btn-outline btn-sm" onclick="returnMyLoan(${loanIdVal})">
                        ...
                        <button class="btn btn-secondary btn-sm" onclick="extendMyLoan(${loanIdVal})">
...
async function returnMyLoan(loanId) {
    try {
        const res = await fetchApi(`/loans/${loanId}/return`, { method: 'PUT' });
        if (res.ok) {
...
async function extendMyLoan(loanId) {
    try {
        const res = await fetchApi(`/loans/${loanId}/extend`, { method: 'PUT' });
        if (res.ok) {
            const msg = await res.text();
